...
#functools是高阶函数模块, 提供常用的高阶函数, 如wraps
...
def get(path):
    ...
    def decorator(func):    #传入参数是函数
        # python内置的functools.wraps装饰器作用是把装饰后的函数的__name__属性变为原始属性
        ...
        return wrapper
    return decorator
# 这样, 一个函数通过@get(path)的装饰器就附带了url信息
...
# 函数的参数fn本身就是个函数, 下面五个函数针对fn函数的参数做一些处理判断
...
# 这个函数判断fn有没有可变的关键词参数(**), 如果有就输出True
...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        if name == 'request':
            ...
            #如果找到了request参数, 又找到了其他参数是POSTIONAL_OR_KEYWORD(不是VAR_POSITIONAL, KEYWORD_ONLY, VAR_KEYWORD参数)
    ...
    def __call__(self, request):
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            ...
            if request.method == 'POST':
                #content_type 是 request提交的消息主体类型, 没有就返回丢失消息主体类型
                ...
                # 以下2中content type都表示消息主体是表单
                elif ct.startswith('application/x-www-form-urlencoded') or ct.startswith('multipart/form-data'):
                    # request.post方法 从request body读取post参数, 即表单信息, 并包装成字典赋给kw变量
            ...
            if request.method == 'GET':
                #request.query_string 表示url中的查询字符串
                ...
                if qs:
                    ...
                    #keep_blank_values默认值为False, 指示是否忽略空白值, TRUE不忽略, FALSE忽略
        ...
        if kw is None:
            ...
            if not self._has_var_kw_arg and self._named_kw_args:
                #下面5行代码的意思是将kw中key为关键字参数的项提取出来保存为新的kw
                ...
                for name in self._named_kw_args:
                    if name in kw:
                        copy[name] = kw[name]
                kw = copy
            #遍历request.match.info(abstract math info), 再把abstract math info 的值加入kw中
            #若其key既存在于abstract math info又存在与kw中, 发出重复参数警告
            #check named arg:
        ...
        try:
            ...
            r = yield from self._func(**kw)  #执行handler模块里的函数
            #print(r)
            return r
...
def add_route(app, fn):
    ...
    if path is None or method is None:  #如果两个属性其中之一没有值, 就会报错
        raise ValueError('@get or @post not defined in %s.' % str(fn))
    #如果函数fn不是一个协程或生成器, 就把这个函数编进协程
...
def add_routes(app, module_name):
    #如果handlers模块在当前目录下, 传入的modlue_name就是handlers
    #如果handlers模块在handler目录下, 传入的modlue_name就是handler.handlers

    #Python rfind() 返回字符串最后一次出现的位置, 如果没有匹配项则返回-1
    #str.rfind(str, beg=0 end=len(string))
    #str -- 查找的字符串
    #beg -- 开始查找的位置, 默认为0
    #end -- 结束查找的位置, 默认为字符串长度
    ...
    if n == (-1):
        ...
        #第一个步骤获取aaa模块的信息
        #第二个步骤通过getattr()方法获取子模块名, 如aaa.bbb
        mod = getattr(__import__(module_name[:n], globals(), locals(), [name]), name)
    #dir()不带参数时, 返回当前范围内的变量,方法,定义的类型列表; 带参数是, 返回参数的属性,方法列表. 如果参数包含方法__dir__(), 发方法将被调用.如果参数不包含__dir__(), 该方法将最大限度地收集参数信息.
    for attr in dir(mod):
        if attr.startswith('_'):
            continue
        #排除私有属性后, 用getattr调用handlers模块里的属性(方法)
        #fn就是handler里的函数
        fn = getattr(mod, attr)
